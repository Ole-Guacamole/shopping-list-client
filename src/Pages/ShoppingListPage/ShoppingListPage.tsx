...
const ShoppingListPage: React.FC = () => {
  ...
  const handleDeleteList = () => {
    setListToDelete(shoppingListId || null);
    (
      document.getElementById("delete_list_modal") as HTMLDialogElement
    )?.showModal();
  };
  ...
  const confirmDeleteList = async () => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_SERVER_URL}/shopping-lists/${listToDelete}`
      );
    ...
    <div className="px-4 pb-4">
      <div className="card prose py-4 px-5 m- w-full bg-base-200">
        ...
        <div className="join w-full my-2">
          ...
          />
          <button className="btn join-item btn-primary" onClick={handleAddItem}>
            ADD ITEM
          </button>
        </div>
      ...
      {/* Modal */}
      <dialog
        id="delete_list_modal"
        className="modal modal-bottom sm:modal-middle"
      >
        <div className="modal-box">
          <h3 className="font-bold text-lg">Confirm Delete</h3>
          <p className="py-4">Are you sure you want to delete this list?</p>
          <div className="modal-action">
            ...
            <button className="btn btn-primary" onClick={confirmDeleteList}>
              Confirm
            </button>
          </div>
        </div>
